...
## 2026-08-25 02:07 `src/llm_insights_api.py` Ollama接続先のPC2→PC3切替コミット（自律発案・引き継ぎ確認）

- 経緯: 2026-08-23にRDP遅延事故（`RdpIdd`ドライバ再クラッシュ＋本パイプラインの14.65GBモデル
  `second_constantine/gpt-oss-u:20b` がPC2でVRAM超過しCPU実行に落ちてRDP操作を圧迫、2026-08-04と同根）の
  恒久対策として `OLLAMA_BASE_URL` 環境変数化のコード変更が書かれていたが、`tasks/todo.md`/`tasks/lessons.md`
  のどこにも記載が無くテスト実施の記録も無かったため、2026-08-24 02:07セッションはコミット対象から除外し
  `隙間時間有効活用\tasks\candidate_pool.md` のブロック中候補表に「`run_daily.bat`(git管理外)に
  `OLLAMA_BASE_URL` が実際に設定されているか確認し、本番適用済みならコミットする」という保留条件付きで記録していた。
- 2026-08-25 02:07セッションで `C:\work\run_daily.bat`（git管理外、daily-tech-trendを起動する上位バッチ）を
  確認したところ42行目に `set "OLLAMA_BASE_URL=http://192.168.11.57:11434"`（PC3）が
  2026-08-23 15:44から設定済みで、以降の定時実行（06/09/12/15/18/21時）は実際にPC3向けで稼働していたと判明した。
  未設定時は `http://127.0.0.1:11434`（PC2）にフォールバックする後方互換設計のため、本番は約1.5日問題なく稼働継続中。
- `pytest tests/ -q` で321件全pass（既存309件から回帰なし、本変更用の専用テストは無いがgetenvフォールバックのみの
  最小変更で挙動が全既存テストと矛盾しないことを確認）を確認の上でコミットした（hash未記入・コミット時に更新予定）。
